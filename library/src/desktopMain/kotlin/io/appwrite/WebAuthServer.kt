    ...
    suspend fun create(
        ...
        return suspendCoroutine { continuation ->
            ...
            if (successHtmlResponse == null) {
                this.javaClass.getResourceAsStream("OAuthSuccess.html")?.use {
                    successHtml = it.toByteArray()
                }
            }

            var failureHtml = failureHtmlResponse
            if (failureHtmlResponse == null) {
                this.javaClass.getResourceAsStream("OAuthFailure.html")?.use {
                    failureHtml = it.toByteArray()
                }
            }
            ...
            if (server == null) {
                ...
                server?.createContext(Url(successPath).fullPath) { http ->
                    ...
                    if (paramMap["key"] == null || paramMap["secret"] == null) {
                        ...
                        val data = mapOf(
                            "key" to paramMap["key"].toString(),
                            "secret" to paramMap["secret"].toString()
                        )
