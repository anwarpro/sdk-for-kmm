...
import io.appwrite.serialization.toJsonElement
...
class Realtime(client: Client) : Service(client), CoroutineScope {
    ...
    private inner class AppwriteWebSocketListener : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            ...
            launch(IO) {
                val message: RealtimeResponse = json.decodeFromString(text)
                when (message.type) {
        ...
        private suspend fun handleResponseEvent(message: RealtimeResponse) {
            val event: RealtimeResponseEvent<Any> =
                json.decodeFromJsonElement(message.data.toJsonElement())
